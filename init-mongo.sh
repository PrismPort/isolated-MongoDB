...
echo "Importing TestSchoolData..."
mongoimport --verbose --host localhost --db TestDB --collection SchoolDataExtended --type json --file /sample-school.json --jsonArray --authenticationDatabase admin -u "$MONGO_INITDB_ROOT_USERNAME" -p "$MONGO_INITDB_ROOT_PASSWORD" || echo "Failed to import TestUserData"
